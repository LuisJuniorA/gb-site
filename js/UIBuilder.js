...
export class UIBuilder {
    ...
    initEventListeners() {
        ...
        bus.on(EVENTS.KEYBINDS_UPDATED, (idMap) => {
            CONTROLS_CONFIG.forEach(ctrl => {
                const physicalKey = Object.keys(idMap).find(key => idMap[key] === ctrl.id);
    ...
    renderList() {
        ...
        const visualOrder = ["ctrl-up", "ctrl-down", "ctrl-left", "ctrl-right", "btn-a", "btn-b", "btn-select", "btn-start"];
        const sorted = [...CONTROLS_CONFIG].sort((a, b) => visualOrder.indexOf(a.id) - visualOrder.indexOf(b.id));

        this.container.innerHTML = sorted.map(ctrl => `
